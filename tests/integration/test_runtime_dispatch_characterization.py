...
class LegacyRuntimeDispatchCharacterizationTests(unittest.TestCase):
    def test_runtime_registry_has_the_current_runtimes_in_discovery_order(self):
        self.assertEqual(
            ...
            ("claude", "codex", "cursor", "opencode", "kiro", "pi", "omp", "hermes"),
        )
        ...
        self.assertNotIsInstance(meter.runtime_registry().get("omp"), LegacyRuntimeAdapter)
    ...
    def test_omp_routes_through_the_native_adapter(self):
        source = {"provider": "omp", "id": "omp-session"}
        expected = {"provider": "omp", "marker": object()}
        adapter = mock.Mock()
        adapter.load.return_value = expected

        with mock.patch.object(meter, "_omp_native_adapter", return_value=adapter):
            self.assertIs(meter.recompute(source), expected)
    ...
    def test_discovery_routes_each_runtime_once_in_registry_order(self):
        rows = {
            ...
            for runtime_id in ("claude", "codex", "cursor", "opencode", "kiro", "pi", "omp", "hermes")
        }
        ...
        omp_adapter = mock.Mock()
        omp_adapter.discover_legacy.return_value = (rows["omp"],)
        ...
        with mock.patch.object(
            ...
            meter, "_omp_native_adapter", return_value=omp_adapter
        ), mock.patch.object(
        ...
        omp_adapter.discover_legacy.assert_called_once()
    ...
    def test_one_discovery_failure_returns_other_runtimes_and_bounded_status(self):
        ...
        omp_adapter = mock.Mock()
        omp_adapter.discover_legacy.return_value = ()
        hermes_adapter = mock.Mock()
